...
- Функциональность:
  - Прием .md файлов и метаданных через gRPC streaming
  - Обработка документов через сервисы фрагментации и эмбеддинга
  - Сохранение в Qdrant: dense vectors (эмбеддинги) + sparse vectors (BM25)
  - Детерминистичное кодирование токенов через MD5 для BM25

### 5. Searcher Service (Поиск)
- Назначение: Семантический поисковый движок с поддержкой гибридного поиска
...
- Функциональность:
  ...
  - MMR re-ranking для разнообразия результатов
...
Система поиска использует комбинацию двух подходов:

```
Запрос → Dense (эмбеддинг) ─┬─→ RRF Fusion → MMR → Результаты
         └→ Sparse (BM25) ──┘
```

**Dense поиск:**
- Векторные эмбеддинги от трансформерной модели (ru-en-RoSBERTa, 1024 измерения)
- Семантическое сходство через косинусную близость в Qdrant
- Находит документы по смыслу, даже если нет точных совпадений слов

**Sparse поиск (BM25):**
- Разряженные вектора на основе частоты токенов
- TF-веса вычисляются при индексации, IDF хранится в Qdrant (Modifier.IDF)
- Находит документы по ключевым словам, точные совпадения

**RRF (Reciprocal Rank Fusion):**
- Qdrant prefetch: dense + sparse → слияние через RRF
- Формула: `score = 1/(k+rank_dense) + 1/(k+rank_sparse)`
- k=60 (стандартное сглаживание)

**MMR (Maximal Marginal Relevance):**
- Постпроцессинг для разнообразия результатов
- Итеративный выбор: `score = λ·relevance - (1-λ)·similarity`
- Jaccard similarity на токенах (быстро, без векторов)
- По умолчанию: λ=0.7 (70% релевантность, 30% разнообразие)

### Конфигурация

**Indexer (создание sparse vectors):**
```json
{
  "qdrant": {
...
| Аспект | Dense | Sparse (BM25) | Гибридный |
|--------|-------|---------------|-----------|
| Семантика | ✅ | ❌ | ✅ |
| Точные совпадения | ❌ | ✅ | ✅ |
| Разнообразие | ❌ | ❌ | ✅ (MMR) |
| Производительность | Быстро | Быстро | Быстро |
