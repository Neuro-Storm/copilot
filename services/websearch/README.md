...
Веб-интерфейс использует централизованную систему авторизации через Auth Service.

**Требуемые роли:**
- Поиск: `admin`, `user`, `viewer`
- Генерация ответов: `admin`, `user`

**Страница входа:** http://localhost:8001/login

**Логирование:** Все поисковые запросы логируются с указанием пользователя:
```
[username] role=user query="..." generation=True ip=192.168.1.1
```
...
WebSearch автоматически использует возможности гибридного поиска Searcher:

- **Dense + Sparse BM25:** Searcher выполняет параллельный поиск по обоим источникам
- **RRF Fusion:** Qdrant автоматически сливает результаты через Reciprocal Rank Fusion
- **MMR Re-ranking:** Опциональное пере-ранжирование для разнообразия результатов

**Никаких изменений в API не требуется** — всё работает прозрачно на уровне Searcher Service.

Для настройки гибридного поиска и MMR измените `services/searcher/config.json`:
